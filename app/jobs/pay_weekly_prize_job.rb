class PayWeeklyPrizeJob < ActiveJob::Base
  queue_as :default

  def perform(*args)
    my_prize_pool = PrizePool.find_by_name('Weekly')
    total_to_pay = my_prize_pool.total
    prize_pool_entries = my_prize_pool.get_the_entries

    if prize_pool_entries.blank?
      puts "Could not find any entries to pay for the weekly prize..."
    else
      ...
      puts "Weekly prize winner: #{winning_entry.sender} who just won #{total_to_pay} BTC!"
